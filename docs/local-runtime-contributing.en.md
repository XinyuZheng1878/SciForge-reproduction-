...
- serve assembly files such as `kun/src/cli/serve-entry.ts` /
  `kun/src/server/runtime-factory.ts`: install all port implementations
  (in-memory or file-backed) and plug them into `buildRouter`.
- `kun/src/cli/serve.ts` is the CLI argument entry point and only parses argv +
  calls `buildRouter`.
...
5. **renderer mapping** (GUI side): `src/renderer/src/agent/agent-runtime-event-dispatcher.ts`
   Dispatch to `sink.onCompactionProgress(...)` through the runtime event dispatcher

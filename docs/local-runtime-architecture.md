...
provider；上游 provider base URL、provider API key、vision translator provider URL
和 internal profile 都属于 Model Router 内部配置，不进入 SciForge Runtime 配置。
...
- 历史消息发送到 Model Router 前会做共享的 model-history repair：孤儿
...
- 连接手机旧 `agentThreadIds.codewhale/reasonix` 会被丢弃；只保留
  `agentThreadIds.sciforge`、`agentThreadIds.codex` 和 `agentThreadIds.claude`
  这类 canonical runtime/thread 映射。

...
只能围绕 SciForge Runtime：产品层面由 `RuntimeHost` 在 `sciforge | codex` 之间选择，SciForge Runtime 仍是
...
这个边界借鉴 TUI/CodeWhale 的 serve HTTP 架构：GUI 不直接嵌 SciForge Runtime agent
...
SciForge Runtime 的缓存命中率要按 Reasonix 的 DeepSeek 原生口径计算和优化：
...
- 设置页 provider selector：Settings -> Agents 可以展示 SciForge Runtime 和 Codex 的配置。
  SciForge Runtime 配置仍包含 binary path、port、autoStart、Model Router base URL、
  runtime API key、runtime token、data dir、public model alias、approval policy、
  sandbox mode、insecure。
...
- `RuntimeHost`：读取 `activeAgentRuntime`，默认选择 `sciforge`，只在用户显式选择
  后委派到 `codex`。
...
- 旧 `deepseek`/`agents.codewhale` 的 port、autoStart、runtime token、
  approval、sandbox 会种到 `agents.sciforge`；旧上游 API key、base URL 和 model
  只可迁移到 Model Router 成员 profile 或要求用户重新配置。
- 旧 `agents.reasonix` 的 API key、base URL、model、autoStart 中，autoStart
  可种到 `agents.sciforge`；上游 provider 字段只可迁移到 Model Router 成员 profile
  或要求用户重新配置。

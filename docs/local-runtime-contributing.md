...
- LLM provider 切换只发生在 Model Router 内；SciForge Runtime 不新增直连
  上游 provider 的 adapter;
...
- `kun/src/cli/serve-entry.ts` / `kun/src/server/runtime-factory.ts`
  等 serve 启动装配文件:把所有 port 的实现(in-memory 或 file-backed)
  装好,塞给 `buildRouter`。
- `kun/src/cli/serve.ts` 是 CLI 参数入口,只负责
...
SciForge Runtime 不直接接入新的 LLM provider。新增 OpenAI-compatible、
DeepSeek-compatible 或其他上游时,扩展 Model Router 的 provider/translator,
然后让 SciForge Runtime 继续使用同一个本地 Model Router `/v1` endpoint。

Runtime 侧只允许做这些配套变更:

1. **profiles**:在 `models.profiles` 中增加新的 router public model alias
   的窗口、能力和压缩阈值。
2. **tests**:用 fake `ModelClient` 或 Model Router 边界 fixture 覆盖
   runtime 行为;不要新增真实上游网络测试。
3. **docs**:更新 `kun/README.md` 和配置文档里的 router alias 示例。

不新增 `modelProvider` 配置,不新增 `openai-compat-model-client.ts` 这类
runtime 直连 adapter,也不在 serve bootstrap 里按 provider 分支注入不同
LLM 客户端。LLM API 只能走 Model Router。
...
5. **renderer 映射**(GUI 端):`src/renderer/src/agent/agent-runtime-event-dispatcher.ts`
   通过运行时事件 dispatcher 给 `sink.onCompactionProgress(...)` 派发
   (若需要)。**这是 GUI 包,不是 kun**。
6. **测试**:
...
- 不要保留与当前原则冲突的 legacy 注释 / 旧 API。发现 stale 兼容说明时,
  在当前 PR 范围内删除或改成明确的历史迁移边界;需要产品取舍时记录到
  `PROJECT.md`,等待人工决策。

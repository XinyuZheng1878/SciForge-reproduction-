...
本文记录 SciForge 中 Kun runtime 的边界和内部约束。它不再把全产品描述为
...
Kun 需要调用模型时，把本地 Model Router `/v1` 当成普通 Responses-compatible
provider；上游 provider base URL、provider API key、vision service URL 和
internal profile 都属于 Model Router 内部配置，不进入 Kun runtime 配置。
...
- 设置页 provider selector：Settings -> Agents 可以展示 Kun 和 Codex 的配置。
  Kun 配置仍包含 binary path、port、autoStart、Model Router base URL、
...
- Model Router 是当前阶段的 LLM provider 边界；SciForge workspace server、
  Browser、Computer Use 等 sidecar 仍不属于这条 Kun runtime contract。
...
- 旧 `deepseek`/`agents.codewhale` 的 port、autoStart、runtime token、
  approval、sandbox 会种到 `agents.kun`；旧上游 API key、base URL 和 model
  只可迁移到 Model Router 成员 profile 或要求用户重新配置。
- 旧 `agents.reasonix` 的 API key、base URL、model、autoStart 中，autoStart
  可种到 `agents.kun`；上游 provider 字段只可迁移到 Model Router 成员 profile
  或要求用户重新配置。
...
- Write：写作助手跟随 `activeAgentRuntime`，Write thread registry 按
  workspace + runtime id 隔离 Kun / Codex 写作线程；inline completion 使用

# SciForge Runtime 缓存优化技术文档

本文记录 SciForge 当前 SciForge Runtime 运行时的缓存优化设计、实现位置、
...
因此 SciForge Runtime 的 token 优化不是单一的“缓存命中率优化”，而是一组组合策略：
...
SciForge Runtime 使用独立的稳定系统提示词文件：
...
SciForge Runtime 通过 `ImmutablePrefix` 管理系统 prompt、tools、pinned constraints
...
工具集合本身就是 prompt prefix 的一部分。SciForge Runtime 在发送请求前会统一做：
...
SciForge Runtime 当前在模型请求边界对消息做一层共享的 model history repair：
...
- SciForge Runtime 会写入一个 error `tool_result`，让模型收敛到更窄的查询或解释原因。
...
GUI 通过 SciForge Runtime 使用统一接口读取 usage：
...
2026-06-02 的 SciForge Runtime 实测结果：
...
这个文档强调一个产品级约束：缓存优化属于 SciForge Runtime，而不是 GUI。
...
- 架构总览：`docs/local-runtime-architecture.md`
- SciForge Runtime 贡献指南：`docs/local-runtime-contributing.md`
- SciForge Runtime 使用说明：`kun/README.md`

...
# 2) point the app at it (the repo's launcher script does this for you):
export SCIFORGE_SCIMODALITY_SERVICE_URL=http://127.0.0.1:3898
```

Check it from the app side: `curl http://127.0.0.1:3898/experts/status` lists each expert's
online state and device.

## When the server is down (graceful, no errors)

If the GPU server is off (or the tunnel is down), the app does a fast ~3s health probe, then **falls
back to feeding the raw file text to DeepSeek**. The turn still completes; the timeline shows
`scientific_modality → raw text — expert service offline`. Text chat and image (Qwen) are
unaffected. Nothing to do — bring the server back up and re-run `deploy/start.sh` to restore experts.

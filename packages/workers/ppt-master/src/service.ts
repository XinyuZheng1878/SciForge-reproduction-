...
export function createPptMasterService(options: PptMasterServiceOptions = {}) {
  ...
  return {
    ...
    async initProject(input: {
      ...
      const initializedProjectPath = parseInitializedProjectPath(init.stdout) ?? projectPath;
      const importResult = input.sourcePaths?.length
        ? await runScript('projectManager', ['import-sources', initializedProjectPath, ...input.sourcePaths, '--move'], {
            cwd: workspaceRoot,
            timeoutMs: DEFAULT_EXPORT_TIMEOUT_MS
          })
        : undefined;
    ...
    async exportPptx(input: {
      ...
      const artifactManifestPath = pptxPath
        ? await writePptMasterArtifactManifest(projectPath, pptxPath)
        : undefined;
      return {
        ...result,
        ...(pptxPath ? { pptxPath } : {}),
...
async function writePptMasterArtifactManifest(projectPath: string, pptxPath: string): Promise<string> {
  const workspaceRoot = inferWorkspaceRootFromProjectPath(projectPath);
...
function parseInitializedProjectPath(stdout: string): string | undefined {
  for (const line of stdout.split(/\r?\n/)) {
    const match = line.match(/(?:Project created:|\[OK\]\s+Project initialized:)\s*(.+?)\s*$/);
    if (match?.[1]?.trim()) return resolve(match[1].trim());
  }
  return undefined;
}

...
let previousImageProvider: string | undefined
...
beforeEach(() => {
  ...
  previousImageProvider = process.env.SCIFORGE_IMAGE_PROVIDER
  ...
  delete process.env.SCIFORGE_IMAGE_PROVIDER
...
afterEach(() => {
  ...
  if (previousImageProvider === undefined) delete process.env.SCIFORGE_IMAGE_PROVIDER
  else process.env.SCIFORGE_IMAGE_PROVIDER = previousImageProvider
...
describe('image generation engine', () => {
  ...
  it('falls back to chat-completions image data URIs for Gemini-style image models', async () => {
    const pngBase64 =
    ...
    process.env.SCIFORGE_IMAGE_MODEL = 'gemini-flash-image-preview'
    const fetchMock = vi.fn(async (input: RequestInfo | URL) => {
      ...
      if (url === 'http://image-provider.local/v1/chat/completions') {
        return new Response(JSON.stringify({
          choices: [
            {
              message: {
                role: 'assistant',
                content: `![image](data:image/png;base64,${pngBase64})`
              }
            }
          ]
        }), {
    ...
    expect(fetchMock.mock.calls.map((call) => String(call[0]))).toEqual([
      ...
      'http://image-provider.local/v1/chat/completions'
    ])
  })
})

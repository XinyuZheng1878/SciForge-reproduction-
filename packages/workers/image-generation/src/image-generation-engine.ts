...
export async function editImageFromCanvasPacket(
  ...
  try {
    ...
    const outputDir = await resolveOutputDir(workspaceRoot, request.outputDir)
    await mkdir(outputDir, { recursive: true })
    const outputs: Array<{ outputPath: string; manifestPath: string; artifactManifestPath: string; provider: 'openai-compatible' | 'placeholder' }> = []
    ...
    return {
      ok: true,
      status: outputs.some((output) => output.provider === 'openai-compatible') ? 'edited' : 'edited_placeholder',
      intents,
      outputs,
      warnings
    }
...
function providerKind(): 'openai-compatible' | 'placeholder' {
  return process.env.SCIFORGE_IMAGE_PROVIDER === 'openai'
    || Boolean(process.env.SCIFORGE_IMAGE_API_KEY || process.env.OPENAI_API_KEY)
    ? 'openai-compatible'
    : 'placeholder'
}
...
async function renderWithProvider(input: ProviderRenderInput): Promise<ProviderRenderResult> {
  if (providerKind() === 'openai-compatible') {
    try {
      await renderWithOpenAiCompatibleProvider(input)
      return { provider: 'openai-compatible', placeholder: false, warnings: [] }
    } catch (error) {
...
async function renderWithOpenAiCompatibleProvider(input: ProviderRenderInput): Promise<void> {
  const apiKey = process.env.SCIFORGE_IMAGE_API_KEY || process.env.OPENAI_API_KEY
  if (!apiKey) throw new ProviderError('Missing SCIFORGE_IMAGE_API_KEY or OPENAI_API_KEY.')
  const baseUrl = (process.env.SCIFORGE_IMAGE_BASE_URL || process.env.OPENAI_BASE_URL || 'https://api.openai.com/v1').replace(/\/$/, '')
  ...
  const errors: string[] = []
  for (const candidateBaseUrl of openAiCompatibleBaseUrlCandidates(baseUrl)) {
    try {
      await renderWithOpenAiImagesEndpoint(candidateBaseUrl, {
        apiKey,
        model,
        prompt,
        size,
    ...
    try {
      await renderWithOpenAiChatImageEndpoint(candidateBaseUrl, {
        apiKey,
        model,
        prompt,
        outputPath: input.outputPath
...
function openAiCompatibleBaseUrlCandidates(baseUrl: string): string[] {
...
async function renderWithOpenAiImagesEndpoint(
  baseUrl: string,
  input: {
    apiKey: string
    model: string
...
async function renderWithOpenAiChatImageEndpoint(
  baseUrl: string,
  input: {
    apiKey: string
    model: string
  ...
  const payload = await parseProviderJson(response, 'Chat image endpoint')
  if (!response.ok) throw new ProviderError(providerHttpError('Chat image endpoint', response.status, payload))
  const message = payload.choices?.[0]?.message
  if (await writeProviderImage(message, input.outputPath)) return
  throw new ProviderError('Chat image endpoint response did not include a data URI, image URL, or image payload.')
}

import type { ComputerUseScreenshotOutput } from './contract.js'
...
export function computerScreenshotOutput(
  input: Omit<ComputerUseScreenshotOutput, 'kind'>
): ComputerUseScreenshotOutput {
  return { kind: 'computer_screenshot', ...input }
}
...
export function capToolResultImages<T extends { kind: string; output?: unknown }>(
  history: T[],
  maxKept: number
): T[] {

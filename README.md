...
**SciForge Runtime**（位于 `kun/` 目录）；高级用户也可以在设置里显式选择
...
Codex 只能由用户显式选择；旧 `runtimeRequest` / `startSse` 只作为兼容
shim 保留，renderer 专用 `codex:*` IPC 已删除。完整 contract 见
...
- 管理 `~/.sciforge/write_workspace` 与多个自定义写作空间；旧版本可能仍有 `~/.deepseekgui/write_workspace` 兼容目录。左侧文件树支持新建、重命名与删除。
...
<p align="center">
  <img src="src/asset/img/clawmode.png" alt="SciForge 连接手机" width="860">
</p>
...
| [kun/README.md](kun/README.md) | SciForge Runtime 包：CLI、env、data dir、HTTP API |

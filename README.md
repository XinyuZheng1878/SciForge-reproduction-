...
![SciForge 三层整体架构](docs/architecture/diagrams/diagram-1.png)
...
SciForge 最精巧的设计是让前端**完全不关心底层跑的是哪个运行时**（见**图 2**）。

![中性 AgentRuntime 契约](docs/architecture/diagrams/diagram-2.png)三个运行时的通信协议截然不同：
...
默认的 SciForge Runtime 是一个**独立的本地 Node.js 进程**，作为 HTTP/SSE server 对外提供服务（见**图 3**）。

![Runtime HTTP/SSE 通信时序](docs/architecture/diagrams/diagram-3.png)它与主进程的交互分两个方向：
...
![自主科研编排系统四阶段闭环](docs/architecture/diagrams/diagram-4.png)
...
这是系统中最具方法论深度的部分（见**图 5**）。

![假设置信度的贝叶斯更新流程](docs/architecture/diagrams/diagram-5.png)

每次实验试验后，假设的置信度按以下公式更新：
...
所有架构图已嵌入文中对应章节，PNG 源文件见 `docs/architecture/diagrams/`：

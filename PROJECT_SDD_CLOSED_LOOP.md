...
- [ ] 引入 R 块解析：`### R-1: 标题 {status}`。
- [ ] 引入验收项解析：`- [ ]` / `- [x]`。
- [ ] 引入 plan covers 标注解析：`(covers: R-1, R-2)`。
- [ ] 计算需求覆盖率、未覆盖需求、完成进度。
- [ ] 根据计划/todo 状态推导 requirement 状态，但自动写回需要受控。
- [ ] 引入 verify prompt，要求 agent 按验收项检查并更新需求文件。
- [ ] 支持需求相关图片/原型附件时，使用 DeepSeek-GUI 自己的目录规范。

## 不引入范围

- [ ] 不使用 `.kunsdd` 作为 DeepSeek-GUI 新目录名。
- [ ] 不引入 Kun 品牌文本。
- [ ] 不强制迁移已有 SDD 草稿。
- [ ] 不引入 UI 插件/iKun。
- [ ] 不引入图片/视频/音乐生成工具。
- [ ] 不改变现有 plan/todo 的主链路。
...
- [ ] 能解析需求文档中的 R 块和验收项。
- [ ] 能解析计划中的 covers 标注并计算覆盖率。
- [ ] 未覆盖需求能在 UI 或报告中明确提示。
- [ ] verify prompt 能要求 agent 按验收项核验。
- [ ] 自动状态写回受控，不会在用户编辑中造成内容冲突。
- [ ] 现有 SDD 草稿能继续读取，不被强制迁移。

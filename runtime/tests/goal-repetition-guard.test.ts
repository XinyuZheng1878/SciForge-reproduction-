...
function failIfGoalLoopEscapes(calls: number, maxCalls: number): void {
  if (calls > maxCalls) {
    throw new Error(`goal repetition guard did not stop within ${maxCalls} model calls`)
  }
}

...
- [x] Remote Channel runtime 内部清名完成：`ClawRuntime` / `createClawRuntime` / `ClawIncomingImMessage*`、`claw-runtime*.ts`、runtime helper/log category 和 IPC schema 本地变量已迁到 `RemoteChannelRuntime` / `createRemoteChannelRuntime` / `RemoteChannelIncomingMessage*` / `remote-channel-runtime*.ts` 语义；DESIGN 路径同步，`openclaw` / `openclaw-weixin` 第三方包名和协议 channel id 继续作为 vendor/protocol 例外保留。

...
- [ ] Remote Channel / Connect Phone 公开 TypeScript surface 已决策破坏式清名：shared typings / functions / constants 从 `Claw*` 直接改为 `RemoteChannel*` / `ConnectPhone*`，不保留 deprecated alias；renderer store 的 `clawChannels` / `activeClawChannelId` / `chat-store-claw-actions` 也纳入同一批内部清名，目标是消除公开 API 与内部状态的旧命名漂移。实现时需先处理现有 `activeRemoteChannelId` 命名冲突：该字段当前是 remote guard 选中 channel，建议先改成 `remoteGuardChannelId`，再把 composer/current channel 的 `activeClawChannelId` 改成清晰的 remote-channel 名；`openclaw-weixin` / `openclaw` 第三方包名、协议 channel id、vendor shim 和第三方 notice 不属于清名范围。

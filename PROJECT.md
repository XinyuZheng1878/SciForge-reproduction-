...
- [ ] sci-modality expert provider 与 image-generation direct provider 原则上统一经 router，避免形成新的 LLM/API 旁路。已决策全部并入 Model Router，不拆新的 Media Router；GUI/runtime 只依赖 Model Router，不直接触碰 provider 密钥。已完成 sci-modality / image-generation 边界防回归、image-generation worker-contained 临时例外说明、runtime env scrub 覆盖 `EDAG_LLM_*`，以及 settings 文案不把 image direct provider 描述为默认路径；待实施：把 image-generation 和 sci-modality expert provider 的临时直连例外收敛进 Model Router 内部能力与 health/config 生命周期。
...
- [ ] Remote Channel / Connect Phone 公开 TypeScript surface 已决策破坏式清名：shared typings / functions / constants 从 `Claw*` 直接改为 `RemoteChannel*` / `ConnectPhone*`，不保留 deprecated alias；renderer store 的 `clawChannels` / `activeClawChannelId` / `chat-store-claw-actions` 也纳入同一批内部清名，目标是消除公开 API 与内部状态的旧命名漂移。
...
- [ ] Paper Radar plugin workspace/core 归属已决策：抽出 shared core，GUI worker 和 MCP 共用，删除 standalone HTTP service/plugin workspace 边界。当前 IPC 已走 `PaperRadarWorkerService`，app-side HTTP sidecar 已删除；但 `packages/workers/paper-radar` 仍依赖 `plugins/paper-radar-service` 的 storage/source/profile/ranking modules，root scripts/packaging 仍保留 standalone HTTP service，待重构清理。

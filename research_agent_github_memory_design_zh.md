...
- 核心行为放在 `.codex/skills/research-memory/SKILL.md`，由 Skill 约束 agent 的确认、证据等级和 GitHub 同步边界。
- GitHub 约定承载协作面：issue 收反馈，PR 承载正式记忆变更，状态文件和 artifact 记录走模板。
- 文件模板放在 `docs/templates/research-memory/`，agent 只基于确认过的信息填写模板。
- 不再提供旧的项目记忆后端服务，也不在 GUI/agent runtime 中自动接入项目记忆服务。
...
## 4. Skill、GitHub 约定、模板、Extension UI 的关系
...
- `status.html` 或 `status.md` 只是状态视图，不是 review 渠道。
...
### 5.2 Research Memory Skill + Templates
...
- CSS 尽量少，可以内联。
- 输出要稳定，方便 PR diff。
- 不作为真实事实源，真实事实仍在本地 workspace 和 `.agents/artifacts.yml`。
...
- `validated`、public claim、中高风险结论必须得到用户明确确认。
- issue/PR/comment/status 更新必须先生成草稿。
...
### 12.2 Draft / Policy / Template Workflow

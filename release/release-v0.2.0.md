...
本版最大的变化，是 DeepSeek GUI 现在以内置自研 Agent 运行时 **SciForge Runtime** 作为唯一核心运行时。
...
- 旧的 CodeWhale / Reasonix / deepseek-runtime 设置会迁移到 SciForge Runtime 配置。
...
SciForge Runtime 负责把模型调用、缓存、上下文和工具成本管住；GUI 负责把需求、计划、目标、审查、写作、自动化和文件改动变得可见、可控、可继续。两者结合之后，DeepSeek GUI 更适合真实项目里的长期工作，也更能把每一个 token 用在刀刃上。

...
打通**真实 trace**:GUI `AgentRuntimeHost` 在 Kun/Codex/Claude 等 runtime turn 完成时把 neutral turn items 喂给引擎。

- **锚点免费**:neutral `AgentRuntimeItem` 已有稳定持久 `item.id`(`src/shared/agent-runtime-contract.ts`)→ 直接当 `trace_ref`,无需改 schema。
- **seam = 一个新模块 + 一个调用点**(镜像 vision/scimodality 约定:env-gated + fail-open):
  - `src/main/runtime/evidence-dag-feed.ts`:`toEvidenceDagTraceItems`(纯映射,丢 approval/error 等噪声)+ `feedEvidenceDag`(POST `/threads/{runtimeId}:{id}/ingest-trace`,Bearer,超时,**永不抛**)。
  - 调用点:`AgentRuntimeHost.subscribeEvents()` 中的 completed `turn_lifecycle`,**fire-and-forget**(不阻塞、不破坏回合)。gate=`SCIFORGE_EVIDENCE_DAG_SERVICE_URL`。
- **测试**:`src/main/runtime/evidence-dag-feed.test.ts` + `src/main/runtime/agent-runtime/host.test.ts`;`npm run typecheck` 和 `kun` 包 `tsc --noEmit` 干净。

...
class TestServerRobustness(unittest.TestCase):
    ...
    def test_thread_route_decodes_runtime_scoped_ids(self):
        trace = {"trace": [{"id": "step-1", "type": "tool_result", "content": "Evidence."},
                           {"id": "step-2", "type": "message", "content": "Claim."}]}
        status, data = self._req("POST", "/threads/codex%3Athread%2Fone/ingest-trace", trace)
        self.assertEqual(status, 200)
        self.assertEqual(data["data"]["summary"]["thread_id"], "codex:thread/one")
        status, data = self._req("GET", "/threads/codex%3Athread%2Fone/graph")
        self.assertEqual(status, 200)
        self.assertEqual(data["data"]["summary"]["thread_id"], "codex:thread/one")

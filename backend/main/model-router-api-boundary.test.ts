...
const sourceRoots = ['backend', 'frontend', 'runtime/src', 'workers', 'scripts']
const guiRuntimeSourceRoots = ['backend/main', 'runtime/src']
const runtimeImageDirectEnvSourceRoots = ['backend/main', 'frontend', 'runtime/src']
...
function isAllowedBoundaryMarker(hit: DirectCallHit): boolean {
  if (hit.file === 'runtime/src/adapters/model/model-error-probe.ts') {
    return hit.marker === 'DeepSeek API host' && hit.text.includes("host === 'api.deepseek.com'")
  }
  if (hit.file === 'runtime/src/contracts/model-endpoint-format.ts') {
    return hit.marker === 'chat completions endpoint' || hit.marker === 'messages endpoint'
  }
  if (hit.file === 'backend/main/upstream-provider-env.ts') {
    return true
  }
  if (
    hit.file === 'backend/main/runtime/claude-code/claude-code-config.ts' &&
    (hit.text.includes('runtime.apiKey') || hit.text.includes('runtime.baseUrl'))
  ) {
    return true
  }
  if (
    hit.file === 'backend/main/local-runtime/local-runtime-process.ts' ||
    hit.file === 'backend/main/runtime/codex/codex-config.ts' ||
    hit.file === 'backend/main/runtime/claude-code/claude-code-config.ts'
  ) {
  ...
  if (
    ...
    (
      hit.file === 'workers/sci-modality-router/src/experts.ts' ||
      hit.file === 'workers/sci-modality-router/provider/server.py'
    )
  ) {
    return true
  }
...
function isAllowedSciModalityBoundaryMarker(hit: DirectCallHit): boolean {
  if (
    hit.file === 'backend/main/model/model-router-sidecar.ts' &&
...
describe('model router API boundary static audit inventory', () => {
  it('keeps the static scan scoped to production code outside the Model Router package', () => {
    ...
    expect(files).toContain('backend/main/index.ts')
    expect(files).toContain('backend/shared/app-settings-types.ts')
    expect(files).toContain('workers/sci-modality-router/src/experts.ts')
    expect(files.some((file) => file.startsWith('workers/model-router/'))).toBe(false)
    expect(files.some((file) => testFilePattern.test(file))).toBe(false)
    expect(hits.every((hit) => !hit.file.startsWith('workers/model-router/'))).toBe(true)
  })
})
...
describe('P7/P8 model router API boundary enforcement', () => {
  ...
  it('keeps ModelRouterModelClient production construction behind the local Model Router runtime factory', () => {
    ...
    expect(directConstructors.map((hit) => hit.file)).toEqual(['runtime/src/server/runtime-factory.ts'])
  })

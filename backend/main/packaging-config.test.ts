...
describe('electron-builder local runtime packaging', () => {
  it('keeps local runtime install/build decisions in one package script', () => {
    ...
    const sqliteModule = join(root, 'runtime/node_modules/better-sqlite3')
  ...
  it('keeps pending release-strategy packages out of bundled app content', () => {
    ...
    for (const rawGlob of [
      'plugins/**/*',
      'workers/sci-modality-router/**/*',
      'workers/evidence-dag/**/*',
      'workers/gui-owl-computer-use/**/*'
    ]) {
  ...
  it('keeps GUI-Owl sidecar secrets and model weights out of release packaging candidates', () => {
    const deniedCandidates = [
      'workers/gui-owl-computer-use/package.json',
      'workers/gui-owl-computer-use/server/serve-gui-owl-32b.sh',
      'workers/gui-owl-computer-use/启动-secrets.local.ps1',
      'workers/gui-owl-computer-use/models/gui-owl.safetensors',
      'workers/gui-owl-computer-use/models/gui-owl.pt',
      'workers/gui-owl-computer-use/models/gui-owl.pth',
      'workers/gui-owl-computer-use/models/gui-owl.gguf'
    ]
    const bundledDirectoryFileSetDirs = bundledDirectoryFileSets()
      .map((entry) => entry.from)
      .filter((entry): entry is string => typeof entry === 'string')
  ...
  it('validates the unpacked local runtime before release artifacts are created', () => {
    ...
    rmSync(join(unpackedRoot, 'runtime/node_modules/zod'), { recursive: true, force: true })
  ...
  it('keeps Paper Radar bundled as a worker-owned core without a plug-in service dependency', () => {
    ...
    expect(paperRadar?.requiredPaths).toEqual(expect.arrayContaining([
      'workers/paper-radar/package.json',
      'workers/paper-radar/src/mcp-server.ts',
      'workers/paper-radar/src/core/service.ts',
      'workers/paper-radar/src/core/storage.ts'
    ]))
  ...
  it('keeps Model Router release requirements independent of Sci Modality', () => {
    ...
    expect(modelRouter?.requiredPaths).toEqual(expect.arrayContaining([
      'workers/model-router/package.json',
      'workers/model-router/src/cli.ts',
      'workers/model-router/src/manifest.ts',
      'workers/model-router/tools/model-router-trace-audit.ts'
    ]))
    expect(modelRouter?.requiredPaths).not.toEqual(expect.arrayContaining([
      'workers/sci-modality-router/package.json'
    ]))
  })
...
describe('root package workspace contracts', () => {
  it('keeps package.json workspaces aligned with the release worker manifest', () => {
    ...
    expect(rootPackage.workspaces).not.toContain('workers/gui-owl-computer-use')
    expect(rootPackage.scripts).toMatchObject({
      'build:local-runtime': 'node ./backend/scripts/local-runtime-package.cjs build',

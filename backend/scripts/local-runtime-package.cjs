...
const LOCAL_RUNTIME_INSTALL_REQUIRED_PATHS = [
  ...
  'workers/multi-agent/dist/index.js',
  'runtime/node_modules/@sciforge/multi-agent/package.json',
  'runtime/node_modules/diff/package.json',
...
const LOCAL_RUNTIME_REQUIRED_PATHS = [
  'runtime/dist/cli/serve-entry.js',
...
const LOCAL_RUNTIME_SQLITE_MODULE_PATH = 'runtime/node_modules/better-sqlite3'
...
function ensureProjectLocalRuntimeInstall(projectRoot = PROJECT_ROOT) {
  ...
  if (!hasProjectLocalRuntimeInstall(projectRoot)) {
    runNpm(['--prefix', 'runtime', 'ci'], {
      cwd: projectRoot,
      label: 'npm --prefix runtime ci'
    })
  }
...
function buildProjectLocalRuntime(projectRoot = PROJECT_ROOT) {
  ...
  runNpm(['--prefix', 'runtime', 'run', 'build'], {
    cwd: projectRoot,
    label: 'npm --prefix runtime run build'
  })
}
...
function prunePackedLocalRuntimeDependencies(appRoot, platform = process.platform) {
  ...
  assertExists(join(runtimeDir, 'node_modules'), 'local runtime node_modules')

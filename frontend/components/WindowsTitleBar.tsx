...
import type { DesktopCommand } from '@shared/sciforge-api'
import {
  resolveKeyboardShortcutBindings,
  type KeyboardShortcutBindingsV1,
  type KeyboardShortcutCommandId
} from '@shared/keyboard-shortcuts'
import sciforgeLogo from '../assets/img/sciforge.png'
...
function currentPlatform(): string {
  return typeof window !== 'undefined' ? window.sciforge?.platform ?? 'unknown' : 'unknown'
}

function defaultRunDesktopCommand(command: DesktopCommand): Promise<void> {
  if (typeof window === 'undefined' || typeof window.sciforge?.runDesktopCommand !== 'function') {
    return Promise.resolve()
  }
  return window.sciforge.runDesktopCommand(command)
}

function defaultOpenLogDir(): Promise<void> {
  if (typeof window === 'undefined' || typeof window.sciforge?.openLogDir !== 'function') {
    return Promise.resolve()
  }
  return window.sciforge.openLogDir().then(() => undefined)
}
...
export function WindowsTitleBar({ platform, actions }: Props): ReactElement | null {
  ...
  const defaultActions = useMemo<WindowsTitleBarActions>(() => ({
    ...
    showAbout: async () => {
      const version =
        typeof window !== 'undefined' && typeof window.sciforge?.getAppVersion === 'function'
          ? await window.sciforge.getAppVersion().catch(() => '')
          : ''
  ...
  return (
    <div ref={rootRef} className="ds-windows-titlebar ds-drag">
      <div className="ds-windows-titlebar-content">
        <img src={sciforgeLogo} alt="" aria-hidden="true" className="ds-windows-titlebar-icon" />

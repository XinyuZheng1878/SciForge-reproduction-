...
function thread(id: string, workspace: string, runtimeId?: NormalizedThread['runtimeId']): NormalizedThread {
  return {
    id,
    runtimeId,
    title: id,
...
describe('write-thread-registry', () => {
  ...
  it('keeps write thread records isolated by runtime', () => {
    const kun = markWriteThread('/Users/zxy/workspace', 'kun-thread', emptyWriteThreadRegistry(), 'kun')
    const registry = markWriteThread('/Users/zxy/workspace', 'codex-thread', kun, 'codex')

    expect(registry.workspaces['/Users/zxy/workspace'].activeThreadId).toBe('kun-thread')
    expect(registry.workspaces['codex:/Users/zxy/workspace'].activeThreadId).toBe('codex-thread')
    expect(isWriteThreadId('kun-thread', registry, 'codex')).toBe(false)
    expect(isWriteThreadId('codex-thread', registry, 'kun')).toBe(false)
    expect(writeWorkspaceForThreadId('codex-thread', registry, 'codex')).toBe('/Users/zxy/workspace')
    expect(
      activeWriteThreadForWorkspace(
        '/Users/zxy/workspace',
        [
          thread('kun-thread', '/Users/zxy/workspace'),
          thread('codex-thread', '/Users/zxy/workspace')
        ],
        registry,
        'codex'
      )?.id
  ...
  it('does not hydrate write assistant threads from another runtime into the active runtime key', () => {
    const kunWriteThread = {
      ...thread('kun-write-thread', '/Users/zxy/write', 'kun'),
      title: WRITE_ASSISTANT_THREAD_TITLE
    }
    ...
    expect(isWriteThreadId('kun-write-thread', registry, 'codex')).toBe(false)
    expect(registry.workspaces['codex:/Users/zxy/write']).toBeUndefined()
  })

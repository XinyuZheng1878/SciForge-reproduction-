...
const DRAFT_PLAN_INTRO = 'SciForge is asking you to draft a GUI-owned implementation plan.'
const REFINE_PLAN_INTRO = 'SciForge is asking you to revise an existing GUI-owned implementation plan.'
...
export function buildDraftPlanPrompt(options: {
  ...
  return [
    ...
    'You are in plan mode. Do not implement, edit project files, change configuration, install dependencies, commit, push, or run destructive commands until the user approves the saved plan.',
    'You may use read-only investigation tools to inspect files and search the workspace before drafting.',
    'If a blocking requirement is missing, use `request_user_input` to ask a small structured question instead of asking as ordinary prose.',
    'Do not use `request_user_input` to ask whether the plan is approved; saving the plan is the approval request.',
    `When the plan is ready, use the \`${GUI_PLAN_CREATE_TOOL_NAME}\` tool to save it with:`,
    ...
    'The saved plan should include context, findings, recommended approach, scope, acceptance criteria, verification, and remaining risks or questions.',
    '',
    'User request:',
...
export function buildRefinePlanPrompt(options: {
  ...
  return [
    ...
    'You are in plan mode. Do not implement, edit project files, change configuration, install dependencies, commit, push, or run destructive commands until the user approves the saved plan.',
    'You may use read-only investigation tools if the feedback requires checking workspace context.',
    'If a blocking requirement is missing, use `request_user_input` to ask a small structured question instead of asking as ordinary prose.',
    'Do not use `request_user_input` to ask whether the plan is approved; saving the revised plan is the approval request.',
    `When the revision is ready, use the \`${GUI_PLAN_CREATE_TOOL_NAME}\` tool to save it with:`,

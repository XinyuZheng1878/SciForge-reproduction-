...
export function resolveSddMarkdownImagePath(
  ...
  if (direct.startsWith('.sciforge/')) {
    return normalizeRelativeParts(direct.split('/'))
  }
...
export async function collectSddDraftImages(input: {
  ...
  const readImage = input.readImage ?? window.sciforge.readWorkspaceImage
  ...
  for (const parsed of parseSddMarkdownImages(input.markdown)) {
    ...
    if (!isSddImageRelativePath(normalizedPath)) {
      errors.push(`SDD images must live under the SDD image directory: ${parsed.markdownPath}`)
      continue
    }

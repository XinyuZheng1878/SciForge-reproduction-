...
describe('Kun single-agent regression', () => {
  it('seeds provider credentials and Kun port from legacy local HTTP settings', () => {
    ...
    expect(migrated.agents?.codex).toEqual(defaultCodexRuntimeSettings())
  ...
  it('JsonSettingsStore saves Kun plus Codex defaults after legacy settings migration', async () => {
    ...
    expect(loaded.agents.kun).toEqual(expect.objectContaining({ port: 8787 }))

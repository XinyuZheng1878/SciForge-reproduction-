...
export function imageGenerationMcpEnv(
  ...
  if (!imageGeneration?.enabled || !imageGeneration.apiKey.trim()) return env
  env.SCIFORGE_IMAGE_PROVIDER = 'openai'
  env.SCIFORGE_IMAGE_API_KEY = imageGeneration.apiKey.trim()
  if (imageGeneration.baseUrl.trim()) env.SCIFORGE_IMAGE_BASE_URL = imageGeneration.baseUrl.trim()

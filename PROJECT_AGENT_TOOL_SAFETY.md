...
- [ ] 工具执行异常转成模型可见 tool result，除真正 abort 外不直接杀掉整个 turn。
- [ ] suppressed / duplicate tool calls 能写入可解释结果，避免 queue 卡住。
- [ ] 重复无工具 goal continuation 有上限和明确停止原因。
- [ ] active turn abort / interrupt 后，open items 能 finalize 为 aborted / failed。
- [ ] per-turn sandbox mode 传入 tool host，并过滤/拦截不允许的工具。
- [ ] plan mode 工具隔离：只暴露 read-only / plan 所需工具。
- [ ] `web_fetch` 大 body 截断为可用摘要，避免因过大直接失败。
- [ ] 使用统一 runtime turn queue，不额外创建旁路执行链路。
...
- [ ] 不引入新工具生态或 UI plugin。
- [ ] 不引入图片/语音/视频生成工具。
- [ ] 不改变审批 UI 的产品语义，除非旧逻辑与 per-turn policy 冲突。
- [ ] 不改变 Model Router 架构。
...
- [ ] 单个工具执行失败后，模型能收到失败结果并继续或收敛。
- [ ] suppressed duplicate tool call 不会让 turn 永久 running。
- [ ] plan mode 下无法执行写入/命令类工具。
- [ ] workspace-write sandbox 下 in-process 文件写入限制在 workspace 内。
- [ ] abort 后 timeline 中未完成 item 有明确终态。
- [ ] web fetch 超大响应不会导致整个 turn 失败。

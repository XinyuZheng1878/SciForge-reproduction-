...
SciForge Runtime 取意于《庄子·逍遥游》中的“北冥有鱼，其名为鲲”。在 SciForge
...
模型窗口、模型能力和模型级压缩阈值写在 `models.profiles`。内置配置已包含 `deepseek-v4-pro`、`deepseek-v4-flash` 以及兼容别名 `deepseek-chat` / `deepseek-reasoner`；DeepSeek V4 默认是 1M 上下文，并在约 980k input tokens 时开始压缩。旧的 `contextCompaction.modelProfiles` 仍会读取以兼容已有配置，但新配置请使用 `models.profiles`。更完整的文件位置、字段格式和用户自定义方式见 `../docs/local-runtime-config.md`。
...
GUI 主流程由 `local-runtime-process.ts` 启动 SciForge Runtime，并通过 `runtimeRequest` 使用 bearer token 调用活动 base URL。`agents.sciforge` 下保存 `binaryPath`、`port`、`autoStart`、`runtimeToken`、`dataDir`、`model`、`approvalPolicy`、`sandboxMode`、`insecure`；凭据与上游 base URL 统一保存在 provider 与 Model Router 设置中。
...
历史 provider 专属设置不再属于当前运行链路；GUI 只保存一个本地 runtime 配置，并通过 provider 设置和 Model Router 处理凭据与上游路由。
